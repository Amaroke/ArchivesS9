...
public class WordCountSort extends Configured implements Tool {

	public static void main(String[] args) throws Exception {
		int exitCode = ToolRunner.run(new Configuration(), new WordCountSort(), args);
	...
	public int run(String[] args) throws Exception {
		...
		if (args.length < 2) {
			System.out.printf("Usage: WordCountSort [generic options] <input dir> <output dir> [options]\n");
			return -1;
		}
		...
		jobCount.setJarByClass(WordCountSort.class);
		...
		jobCount.setReducerClass(SumReducer.class);
		...
		jobSort.setJarByClass(WordCountSort.class);
		jobSort.setJobName("Word-Count-Sort");
		...
		jobSort.setReducerClass(InverseReducer.class);
